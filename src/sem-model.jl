...
function do_all_sems()
    wave3 = load_dall_v3()
    wave3.Managing_Financially =  map_Managing_Financially.( wave3.Managing_Financially )
    for o in union(observed_vars1, observed_vars2)
        println( "on $o")
        wave3[!,o] = extract_number.( wave3[!,o] )
    end
    wave4 = CSV.File( joinpath( DATA_DIR, "national-w-created-vars.tab")) |> DataFrame 
    #
    # Cast weights to StatsBase weights type. Not used ATM.
    #
    wave4.weight = Weights(wave4.weight)
    wave4.probability_weight = ProbabilityWeights(wave4.weight./sum(wave4.weight))
    #
    # this thing only likes numerical values, I think, so...
    for o in union(observed_vars1, observed_vars2)
        wave4[!,o] = extract_number.( wave4[!,o] )
    end

    do_one_years_SEMS( wave4 )
    do_one_years_SEMS( wave3 )
